...
def analyze_duplex_structure_rnafold(seq1, seq2, filename, project_dir):
    ...
    fnull = open(devnull, 'w')   #this line is necessary to omit output generated by UNAFOLD
    call("RNAduplex < "+s12_file+" > "+ filename + ".rnafold.out", shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by RNAfold
...
def analyze_duplex_mfe_rnafold(filename, project_dir):
    ...
    if path.exists(project_dir+"/structures/"+filename+".rnafold.out"):
        try:
            output = check_output(["tail", "-n 1", "structures/" + filename + ".rnafold.out"]).rstrip()
...
def analyze_structure_rnafold(seq, filename, project_dir):
    ...
    call("RNAfold -i " + filename +".seq --noPS -d2 > " + filename + ".rnafold.out", shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by RNAplfold
