...
def analyze_terminator(seq, outdir):
    transterm_files = os.path.join(outdir, 'transterm_files')
    if not os.path.exists(transterm_files):
        os.mkdir(transterm_files)
    ...
    file1_path = os.path.join(transterm_files, file1)
    file2_path = os.path.join(transterm_files, file2)

    system("echo '>seq1\n{}' > {}".format(str(seq), file1_path))
    system("echo 'fakegene1    1 2    seq1\nfakegene1    {} {}    seq1' > {}").format(str(len(seq) - 1), str(len(seq)),
                                                                                      file2_path)

    # run transtermhp
    output = Popen(
        "/vagrant/codon_optimization/d-tailor/3rdParty/transterm -p /vagrant/codon_optimization/d-tailor/3rdParty/transterm/expterm.dat tmp/transterm_files/" + file1 + " tmp/transterm_files/" + file2 + " 2> tmp/transterm_files/err.txt | awk /TERM/'{print $8}'",
...
def analyze_duplex_structure_rnafold(seq1, seq2, filename, project_dir):
    ...
    s12_file = os.path.join(project_dir, str(r) + "s1.s2.seq")
    rnafold_outfile = os.path.join(structures_dir, filename + ".rnafold.out")

    system("echo '>s1\n" + str(seq1) + "\n>s2\n"+str(seq2)+"\n' > " + s12_file)

    fnull = open(devnull, 'w')   #this line is necessary to omit output generated by UNAFOLD
    call("RNAduplex < " + s12_file + " > " + rnafold_outfile, shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by RNAfold
    fnull.close()
...
def analyze_duplex_mfe_rnafold(filename, project_dir):
    if not os.path.exists(project_dir):
        os.mkdir(project_dir)
    ...
    if path.exists(rnafold_outfile):
        try:
...
def analyze_structure(seq, filename, project_dir, ensemble=False):
    if not os.path.exists(project_dir):
        os.mkdir(project_dir)
    ...
    outfile = os.path.join(project_dir, filename + '.seq')
    # print(outfile)
    ...
    fnull = open(devnull, 'w')  # this line is necessary to omit output generated by UNAFOLD
    if ensemble:
        call("./3rdParty/unafold/UNAFold.pl -n RNA " + filename + ".seq", shell=True)#, stdout=fnull,
             # stderr=fnull)  # code is necessary to omit output generated by UNAFOLD
    else:
        call("./3rdParty/unafold/hybrid-ss-min -n RNA " + filename + ".seq", shell=True)#, stdout=fnull,
             # stderr=fnull)  # code is necessary to omit output generated by UNAFOLD

    if os.path.isfile(filename + ".ct"):
        system("mv {}*.ct tmp/structures/".format(filename))
        # remove tmp files
...
def analyze_structure_rnafold(seq, filename, project_dir):
    if not os.path.exists(project_dir):
        os.mkdir(project_dir)
    if not os.path.exists(os.path.join(project_dir, 'structures')):
        os.mkdir(os.path.join(project_dir, 'structures'))
    ...
    call("RNAfold -i " + outfile + " --noPS -d2 > " + os.path.join(project_dir, 'structures', filename + ".rnafold.out"), shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by RNAplfold

    # if os.path.isfile(filename+".rnafold.out"):
    #     system("mv %s*.rnafold.out structures/" % filename)
        # remove tmp files
...
def analyze_structure_mfe(filename, project_dir):
    data = {}
    ...
    if path.exists(project_dir+"/tmp/structures/"+filename+".ct"):
        ...
        print('Cannot find input .ct file, not calculating structure MFEs')
        data['StructureMFE'] = 0

    return data

def analyze_structure_mfe_rnafold(filename, project_dir):
    data = {}

    rnafold_outfile = os.path.join(project_dir, 'structures', filename+".rnafold.out")
...
def look_for_terminators(seq, outdir):
    transterm_files = os.path.join(outdir, 'transterm_files')
    if not os.path.exists(transterm_files):
        os.mkdir(transterm_files)
    ...
    system("echo '>seq1\n{}' > {}".format(str(seq), file1_path))
    system("echo 'fakegene1    1 2    seq1\nfakegene1    {} {}    seq1' > {}".format(str(len(seq) - 1),
                                                                                      str(len(seq)),
                                                                                      file2_path))

    # run transtermhp
    output = Popen(
            "/vagrant/codon_optimization/d-tailor/3rdParty/transterm/transterm -p /vagrant/codon_optimization/d-tailor/3rdParty/transterm/expterm.dat {} {} 2> {} | awk /TERM/'{{print $8}}'".format(file1_path, file2_path, os.path.join(transterm_files, 'err.txt')),
            stdout=PIPE, stderr=None, shell=True).stdout.read()
